...
bool net_ethernet_send(const uint8_t destination[NET_ETHERNET_ADDRESS_SIZE], uint16_t ether_type,
    ...
    for (uint64_t index = 0; index < NET_ETHERNET_ADDRESS_SIZE; ++index) {
        ...
        frame[NET_ETHERNET_ADDRESS_SIZE + index] = interface.mac_address[index];
    }
    ...
    if (!e1000_send_frame(frame, ETHERNET_HEADER_SIZE + payload_size)) {
        ++frames_dropped;
        return false;
    }
    ++frames_sent;
...
void net_poll(void) {
    ...
    for (uint64_t packet = 0; packet < ETHERNET_POLL_BUDGET; ++packet) {
        uint64_t size;
        if (!e1000_receive_frame(frame, sizeof(frame), &size)) {
            break;
        }
