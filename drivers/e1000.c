...
/*
 * Intel e1000 legacy descriptor mode.  It intentionally starts in polling
 * mode: this keeps the first NIC driver deterministic and makes it usable
 * before MSI/MSI-X and a complete Ethernet stack exist.
 */
enum {
    ...
    /* The 82540EM BAR used by QEMU occupies 128 KiB. */
...
/* Allocate a buffer that lies entirely inside one mapped physical page. */
...
bool e1000_initialize(void) {
    ...
    if (device == 0) {
        /* Optional built-in driver: absence must not prevent a real machine booting. */
        spinlock_unlock_irqrestore(&e1000_lock, flags);
        return true;
    }
...
bool e1000_send_frame(const uint8_t *frame, uint64_t size) {
    ...
    uint64_t flags = spinlock_lock_irqsave(&e1000_lock);
    if (!e1000.initialized || (e1000.tx_ring[e1000.tx_next].status & E1000_TX_STATUS_DONE) == 0) {
        spinlock_unlock_irqrestore(&e1000_lock, flags);
        return false;
    }
