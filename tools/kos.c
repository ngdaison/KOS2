...
static const char *readobj_command(void) {
    if (file_exists("C:/Program Files/LLVM/bin/llvm-readobj.exe")) {
        return "C:/Program Files/LLVM/bin/llvm-readobj.exe";
    }
    return "llvm-readobj";
}
...
static int build_kernel(bool kernel_only, bool panic_test) {
    ...
    const char *const flags =
        ...
        "-fno-unwind-tables -Wall -Wextra -Werror -O2 -g -I include "
        "-I third_party/limine-protocol/include -I third_party/font8x8";
    ...
    if (execute_format("call \"%s\" --program-headers build/kos.elf", readobj_command()) != 0 ||
        execute_format("call \"%s\" build/kos.elf", nm_command()) != 0) {
        return 1;
    }
    printf("Kernel ELF built: build/kos.elf\n");
    if (kernel_only) {
        return 0;
    }
...
static int run_qemu(int memory_mb, bool display, bool gdb_wait) {
    ...
    return execute_format("call \"%s\" -machine q35 -m %d "
...
static void print_usage(void) {
    ...
    puts("run options: --display, --gdb-wait, --memory-mb <64..4096>");
}
...
int main(int argc, char **argv) {
    ...
    if (strcmp(argv[1], "run") == 0) {
        if (memory_mb < 64 || memory_mb > 4096) return fail("memory must be between 64 and 4096 MB");
        return run_qemu(memory_mb, display, gdb_wait);
    }
    print_usage();
    return 1;
}
